...
The package is pure TypeScript and ships typed command groups (`source`, `remote`, `updater`) so you rarely call `invoke()` directly. For how the native host exposes these commands, see [Platform integration](/docs/guide/core-concepts/glue-to-native); for the matching native API, see the [Node API reference](/docs/references/javascript/node).
...
For the full updater surface, see the [updater reference](/docs/references/javascript/bridge/updater).
