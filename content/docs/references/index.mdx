...
<Callout type="info">
  Building an app? Start with the [platform integration guide](/docs/guide/platform-integration) and pick your platform: [Electron](/docs/guide/platforms/electron), [Tauri](/docs/guide/platforms/tauri), [Android](/docs/guide/platforms/android), [iOS](/docs/guide/platforms/ios), or [Deno Desktop](/docs/guide/platforms/deno).
</Callout>
...
The bridge (`@wvb/bridge`) runs inside the webview and lets your web app call the native host. It exposes a single `invoke()` function plus typed `source`, `remote`, and `updater` helpers, so the same code works across Electron, Tauri, Android, and iOS — the per-platform transport is abstracted away.
...
<Callout type="warn">
  The mobile bindings are pre-release and not yet published to Maven Central or tagged for Swift Package Manager. Install from source for now.
</Callout>
