...
![Build output is packed into a .wvb archive, served to the webview, and updated over the air from a remote server.](https://static.wvb.dev/diagrams/pack-serve-update.svg)
...
<Tabs items={['npm', 'pnpm', 'yarn']}>
  <Tab value="npm">```sh npm install --save-dev @wvb/cli ```</Tab>
  <Tab value="pnpm">```sh pnpm add -D @wvb/cli ```</Tab>
  <Tab value="yarn">```sh yarn add -D @wvb/cli ```</Tab>
</Tabs>
...
<Callout type="info">
  ...
  package. The Android (Kotlin) and iOS (Swift) bindings are built from the core via UniFFI and are
  **pre-release**: they are not yet published to Maven Central or tagged for Swift Package Manager,
  so install from source for now. See [Platform Support](/docs/guide/platform-support) for the full
  status.
</Callout>
