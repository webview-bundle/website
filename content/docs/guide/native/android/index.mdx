...
<Callout type="info">
  See [Platform support](/docs/guide/getting-started/platform-and-framework-support) for the status
  of every platform.
</Callout>
...
If a handler throws, the library synthesizes a `500 text/plain` response and calls the optional `onError` callback. See [Protocol handling](/docs/guide/core-concepts/protocol-handling) for how a request path maps to a file inside a `.wvb`.
...
See [Bundle sources](/docs/guide/core-concepts/bundle-source) for the manifest format.
...
For the remote HTTP contract, integrity, and signatures across platforms, see [Remote bundles](/docs/guide/core-concepts/over-the-air) and the guide to [building a remote](/docs/guide/remote).
