...
When unset, the builtin dir defaults to `bundles` in the app's resource directory and the remote dir to `bundles` in app local data. Each `.protocol(...)` call registers its own asynchronous URI scheme. See [Protocol handling](/docs/guide/core-concepts/protocol-handling) for how the schemes resolve requests.
...
For the full packing workflow and flags, see the [CLI reference](/docs/references/cli) and [Bundle sources](/docs/guide/core-concepts/bundle-source).
...
<Cards>
  <Card
    ...
    description="How the bundle and local schemes resolve webview requests."
  />
  <Card
    title="Remote bundles"
    href="/docs/guide/core-concepts/over-the-air"
    description="OTA updates, integrity, and signature verification."
  />
  <Card
    title="CLI reference"
    href="/docs/references/cli"
    description="Pack, serve, and deploy bundles from the command line."
  />
  <Card
    title="wvb-tauri source"
    href="https://github.com/webview-bundle/webview-bundle/tree/main/packages/tauri"
    description="The plugin crate on GitHub."
  />
</Cards>
