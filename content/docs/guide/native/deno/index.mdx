...
`@wvb/deno-desktop` is the integration layer; `@wvb/deno` is the FFI peer of [`@wvb/node`](/docs/references/javascript/node) that drives the shared Rust core.
...
`@wvb/deno` is the FFI peer of [`@wvb/node`](/docs/references/javascript/node). It loads a prebuilt Rust `cdylib` through `Deno.dlopen` and exposes the same core classes: `BundleProtocol`, `LocalProtocol`, `Remote`, `BundleSource`, and `Updater`.
...
For the full class and method reference, see the [Deno API reference](/docs/references/javascript/deno).
...
<Cards>
  ...
  <Card
    ...
    description="Where each platform stands, including experimental ones."
  />
  <Card
    title="Remote bundles"
    href="/docs/guide/core-concepts/over-the-air"
    description="How updating, integrity, and signatures work across platforms."
  />
</Cards>
