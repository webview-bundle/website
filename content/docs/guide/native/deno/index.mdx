...
The window points at a single local origin served by `Deno.serve`, so the integration is **single-origin**: it allows exactly one protocol. See [Platform integration](/docs/guide/glue-to-native) for how the core reaches each platform and [Protocol handling](/docs/guide/protocol-handling) for the protocol model.
...
For how integrity and signatures work across platforms, see [Remote bundles](/docs/guide/over-the-air).
