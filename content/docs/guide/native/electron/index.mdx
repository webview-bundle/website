...
<Callout type="info">
  ...
  See [Protocol handling](/docs/guide/core-concepts/protocol-handling) for the full model.
</Callout>
...
<Callout type="info">
  ...
  the [Node API reference](/docs/references/javascript/node).
</Callout>
...
`SignatureAlgorithm` is one of `ecdsaSecp256R1`, `ecdsaSecp384R1`, `ed25519`, `rsaPkcs1V15`, `rsaPss`. See [Remote bundles](/docs/guide/core-concepts/over-the-air) for integrity and signature details, and [Building a remote](/docs/guide/remote) for standing a server up — including a local one for testing.
...
Downloaded versions take priority over builtin ones, so an installed update is served automatically after `updater.download(...)` and `updater.install(...)`. See [Bundle sources](/docs/guide/core-concepts/bundle-source) for resolution details.
