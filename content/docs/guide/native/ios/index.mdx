...
exposed under the `WebViewBundle` module. See [Platform integration](/docs/guide/core-concepts/glue-to-native)
for how the shared core reaches each platform.

Add the package as a dependency (the install script below wires in the native FFI):
...
[Protocol handling](/docs/guide/core-concepts/protocol-handling) for the model.
...
Each app reads from two sources. See [Bundle sources](/docs/guide/core-concepts/bundle-source) for the full model.
...
[Remote bundles](/docs/guide/core-concepts/over-the-air) and [Building a remote](/docs/guide/remote).
