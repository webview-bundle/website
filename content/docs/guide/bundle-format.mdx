...
integrity-checked archive. It is three sections written back to back:
...
| Section          | Holds                                                   |
| ---------------- | ------------------------------------------------------- |
| Header           | magic number, format version, index size, checksum      |
| Index            | path → offset/length/headers entry map                  |
| Data             | LZ4-compressed file contents, sorted by path            |
...
<Callout type="warn">
The format uses **SHA-2** for integrity, not SHA-3. The two jobs are distinct: xxHash-32 protects bytes
at rest, while SHA-2 integrity verifies what was downloaded. See
[Remote bundles](/docs/guide/remote-bundles) for how integrity and signatures are checked during an update.
</Callout>
