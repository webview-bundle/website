...
<Callout type="warn">
  Deno Desktop is **experimental / preview** and not production-ready. The source lives on an
  un-merged branch; `main` ships only a prebuilt dylib artifact. The JSR packages `@wvb/deno` and
  `@wvb/deno-desktop` are published at version **`0.0.0`** and their APIs may change. Treat these
  snippets as preview, not a stable contract.
</Callout>
