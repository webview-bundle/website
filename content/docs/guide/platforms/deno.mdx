...
<Callout type="warn">
Deno Desktop is **experimental / preview** and not production-ready. The source lives on an un-merged branch; `main` ships only a prebuilt dylib artifact. The JSR packages `@wvb/deno` and `@wvb/deno-desktop` are published at version **`0.0.0`** and their APIs may change. Treat these snippets as preview, not a stable contract.
</Callout>
...
```ts title="app.ts (in the webview, preview)"
...
```ts title="dispose.ts (preview)"
// Explicit cleanup.
const protocol = bundleProtocol({ scheme: "app" });
const res = await protocol.handle("get", "app://my-app/index.html");
protocol.free();

// Or scope-bound cleanup with `using`.
{
  using scoped = bundleProtocol({ scheme: "app" });
