...
<Callout type="warn">
  `@wvb/electron-builder` lives in the
  [repository](https://github.com/webview-bundle/webview-bundle/tree/main/packages/electron-builder)
  but is **not yet published to npm** (version `0.0.0`). Install it from source or pin the workspace
  version for now. The manual `extraResources` setup below works without it.
</Callout>
