...
<Callout type="warn">
  Android and iOS are implemented and end-to-end tested, but the bindings are pre-release. They are
  not yet published to Maven Central, and there is no Swift Package Manager tag — install from
  source for now. The minimum supported iOS version is iOS 16. The currently committed xcframework
  contains a simulator-only slice; the device slice is pending. See the
...
<Cards>
  ...
  <Card
    ...
    description="Consume the UniFFI Kotlin bindings (pre-release)."
  />
  <Card
    title="iOS guide"
    href="/docs/guide/platforms/ios"
    description="Consume the UniFFI Swift bindings (pre-release, iOS 16+)."
  />
  <Card
    title="Deno guide"
