...
<Callout type="info">
The bundle protocol supports `GET` and `HEAD`, replays the headers stored for each file, and answers `Range` requests for media with `206`. See [Protocol handling](/docs/guide/protocol-handling) for the full request mapping.
</Callout>
...
<Callout type="info">
Integrity is SHA-2, not SHA-3. The signature covers the integrity string, not the raw archive bytes, and requires an integrity value to be present. See [Remote bundles](/docs/guide/remote-bundles) for the verification flow and [Remote, integrity & signature config](/docs/config/remote) for the schema.
</Callout>
