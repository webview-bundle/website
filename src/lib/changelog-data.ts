...
import { CHANGELOG_REPO, type GhRelease, parseReleases, type Release } from './changelog';
...
export const fetchReleases = createServerFn({ method: 'GET' }).handler(
  async (): Promise<ChangelogData> => {
    try {
      ...
      // Sentry's function middleware still records this; the UI shows its
      // "couldn't load" state instead of a 500.
